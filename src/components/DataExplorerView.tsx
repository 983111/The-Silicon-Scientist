...
const trendData = [
  { iter: 1, evalScore: 0.30, confidence: 15.2 }, { iter: 2, evalScore: 0.45, confidence: 14.8 },
  { iter: 3, evalScore: 0.38, confidence: 14.9 }, { iter: 4, evalScore: 0.61, confidence: 13.5 },
  { iter: 5, evalScore: 0.58, confidence: 13.8 }, { iter: 6, evalScore: 0.79, confidence: 12.1 },
  { iter: 7, evalScore: 0.94, confidence: 11.5 }, { iter: 8, evalScore: 0.91, confidence: 11.8 },
  { iter: 9, evalScore: 0.97, confidence: 11.2 }, { iter: 10, evalScore: 0.99, confidence: 11.0 },
];
...
const TABLE_DATA = [
  { time: '12:04:22.001', id: 'DATA_P_042', duration: '1.442ms', vector: '[0.12, 0.44, 1.0]', status: 'STABLE', statusCls: 'green' as const },
  { time: '12:04:22.048', id: 'DATA_P_043', duration: '1.448ms', vector: '[0.12, 0.45, 0.9]', status: 'STABLE', statusCls: 'green' as const },
  { time: '12:04:22.112', id: 'LOG_N_011', duration: '0.221ms', vector: '[0.88, 0.12, 0.2]', status: 'PROCESSING', statusCls: 'blue' as const },
  { time: '12:04:22.350', id: 'VAL_SYS_992', duration: '8.122ms', vector: '[0.00, 0.00, 0.1]', status: 'REJECTED', statusCls: 'red' as const },
  { time: '12:04:22.601', id: 'DATA_P_044', duration: '1.449ms', vector: '[0.13, 0.46, 1.0]', status: 'STABLE', statusCls: 'green' as const },
  { time: '12:04:22.803', id: 'PARAM_T_007', duration: '0.098ms', vector: '[0.50, 0.50, 0.5]', status: 'PROCESSING', statusCls: 'blue' as const },
];

const SCRIPTS = [
  { name: 'monte_carlo_sim.py', desc: 'Iterative sampling for hypothesis valuation', active: true },
  { name: 'gradient_descent.py', desc: 'Optimization routine analysis' },
  { name: 'evaluate_metrics.py', desc: 'Metric evaluation of dynamic targets' },
  { name: 'model_opt.py', desc: 'Hyperparameter geometry optimization' },
];

function downloadCSV() {
  const header = 'Timestamp,Entity ID,Processing Time (ms),Vector,Status\n';
  const rows = TABLE_DATA.map(r => `${r.time},${r.id},${r.duration},"${r.vector}",${r.status}`).join('\n');
...
export function DataExplorerView() {
  const [chartView, setChartView] = useState<'evalScore' | 'confidence'>('evalScore');
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 18, marginBottom: 20 }}>
        ...
        <div className="card" style={{ padding: '18px 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 12, color: 'var(--ink-muted)', margin: '3px 0 0' }}>Score vs Hyperparameter scatter plot</p>
            </div>
          ...
          <div style={{ height: 280 }}>
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 10, right: 10, bottom: 10, left: -15 }}>
                ...
                <XAxis type="number" dataKey="x" name="Parameter Alpha" tick={{ fontSize: 11, fill: '#7a7870', fontFamily: 'DM Mono' }} axisLine={false} tickLine={false} />
                <YAxis type="number" dataKey="y" name="Eval Score" tick={{ fontSize: 11, fill: '#7a7870', fontFamily: 'DM Mono' }} axisLine={false} tickLine={false} />
                <ZAxis type="number" dataKey="z" range={[20, 150]} name="Confidence" />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div className="card" style={{ padding: '16px 18px' }}>
            ...
            {/* Bar rows */}
            {[
              { label: 'Hypothesis Alpha', value: '94.2%', pct: 94, color: 'var(--accent-sage)' },
              { label: 'Hypothesis Beta', value: '88.7%', pct: 88, color: 'var(--accent-slate)' },
              { label: 'Baseline', value: '42.1%', pct: 42, color: 'var(--accent-gold)' },
            ].map((m, i) => (
            ...
            <div style={{ marginTop: 12, height: 80 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={trendData} margin={{ top: 5, right: 0, left: -30, bottom: 0 }}>
                  ...
                  <Area type="monotone" dataKey={chartView === 'evalScore' ? 'evalScore' : 'confidence'} stroke="#c0392b" strokeWidth={1.5} fill="url(#deGrad)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--rule)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            <div style={{ display: 'flex', gap: 4 }}>
              {['all', 'stable', 'processing', 'rejected'].map(f => (
            ...
            <tbody>
              {filteredTable.map((row, i) => (
                <tr key={i}>
                  ...
                  <td className="mono-cell">{row.duration}</td>
                  <td className="mono-cell" style={{ maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.vector}</td>
